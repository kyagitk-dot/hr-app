...
// line-webhook.ts から classifyIntent() → routeFreeText() を呼び出して使う
...
const ADMIN_NAMES = ['八木幸平'];                 // エスカレーション先（LINE連携時の表示名）
...
export async function classifyIntent(text: string, inSession: boolean): Promise<Intent> {
  const system = `携帯ショップの社員がLINEで送ってきた文章を分類します。次のどれか1語だけを返してください。
...
export async function handleConsult(text: string, lineUserId: string, userName: string, uid: string | null): Promise<string> {
  ...
  const system = `あなたは株式会社Athha（携帯電話販売代理店）の社員一人ひとりを支える、頼れる先輩のようなAIアシスタントです。
...
- 本人を否定せず、でも必要なことは率直に言う。長々と説教しない
...
【社長（八木幸平）に伝えるべき場合 = escalate: true】
- 退職・転職を考えている、心身の不調、ハラスメント、金銭トラブル、取引先との深刻なトラブル、法令やコンプライアンスに関わること
  ...
  if (escalate && summary) {
    try {
      ...
      for (const d of lineSnap.docs) {
        if (ADMIN_NAMES.includes(d.data().displayName)) {
...
/**
 * 件数報告でもコマンドでもない自由文の入口（intent 判定済みのものを振り分ける）。
 */
export async function routeFreeText(intent: Intent, text: string, lineUserId: string, userName: string, uid: string | null): Promise<string> {
  if (intent === 'memo') return await handleWorkMemo(text, lineUserId, userName);
  return await handleConsult(text, lineUserId, userName, uid);
}
