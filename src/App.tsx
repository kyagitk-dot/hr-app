...
const SalesManagerDash = ({allReports, users}) => {
  ...
  useEffect(()=>{
    const unsub = onSnapshot(collection(db,"checkoutPhotos"), snap=>{
  ...
  return (
    <div>
      ...
      <div style={{display:"flex",gap:4,background:C.gray[50],borderRadius:10,padding:4,marginBottom:14,flexWrap:"wrap"}}>
        {[{id:"checkin",label:"入店状況"},{id:"daily",label:"日別一覧"},{id:"carrier",label:"キャリア別"},{id:"agency",label:"代理店・店舗別"},{id:"ranking",label:"メンバー別"},{id:"photos",label:"現場写真"}].map(t=><button key={t.id} style={tabStyle(t.id)} onClick={()=>setTab(t.id)}>{t.label}</button>)}
      </div>
      {tab==="checkin"&&(
        <Card>
      ...
      {tab==="photos"&&(
        <Card>
          <CardTitle>現場写真（日曜日の退勤報告）</CardTitle>
          {checkoutPhotos.length===0?(
            <div style={{textAlign:"center",padding:"20px",color:C.gray[400],fontSize:13}}>まだ写真がありません</div>
          ):(
            <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(140px,1fr))",gap:10}}>
              {checkoutPhotos.map(p=>(
                <a key={p.id} href={p.photoUrl} target="_blank" rel="noreferrer" style={{display:"block",textDecoration:"none"}}>
                  <img src={p.photoUrl} alt={p.displayName} style={{width:"100%",aspectRatio:"1",objectFit:"cover",borderRadius:8,border:`0.5px solid ${C.gray[100]}`,display:"block"}}/>
                  <div style={{fontSize:11,color:C.gray[800],marginTop:4,fontWeight:500}}>{p.displayName}</div>
                  <div style={{fontSize:10,color:C.gray[400]}}>{p.date}</div>
                </a>
              ))}
            </div>
          )}
